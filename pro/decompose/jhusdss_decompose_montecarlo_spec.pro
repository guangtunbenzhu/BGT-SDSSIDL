...
pro jhusdss_decompose_montecarlo_spec, objs, nmfver=nmfver, overwrite=overwrite, $
...
;; load and interpolate the spec
    jhusdss_load_interp_montecarlo_spec, nmfver, thisobjs, loglam=loglam, $
       allflux=allflux, allivar=allivar, $
       normminwave=norm_minwave, normmaxwave=norm_maxwave, $
       normoption=1, boss=boss
    ...
    if (~keyword_set(qaonly)) then begin
       jhusdss_decompose_montecarlo_writeout, thisobjs, nmfver=nmfver, $
          basisfile=basis_file, loglam=loglam, nmf_continuum=nmf_continuum, $
          med_continuum=med_continuum, residual=residual, flux=allflux, $
          ivar=allivar, eigen_values=eigen_values, overwrite=overwrite, boss=boss
    endif else begin
    ;; make the qaplots
