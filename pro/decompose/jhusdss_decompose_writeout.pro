...
pro jhusdss_decompose_writeout, objs, nmfver=nmfver, basisfile=basisfile, $
       ...
       path=path, overwrite=overwrite, boss=boss, dr12=dr12

;; path
if (~keyword_set(path)) then begin
   if (keyword_set(dr12)) then begin
          path=jhusdss_get_path(/nmfqso)+'/'+$
             string(nmfver, format='(I3.3)')+'/Decompose_DR12'
   endif else begin
