pro jhusdss_qaplot_example_bad, nmfver, choose=choose

linename=['Mg_{ }I', 'Mg_{ }II', 'Fe_{ }II', 'Fe_{ }II'];, 'Al II', 'C IV', 'Si II', 'Si IV', 'C II', 'O I'] 
linewave=[2870.,  2780.,   2590.,   2360.];,   1661.,   1560.,  1507.,   1390., 1330.,  1290.]
...
if keyword_set(choose) then begin
   ...
   ;; Select 3 examples
   i1 = where(qso_trim.nabs eq 1 and qso_trim.ew[5, 0] gt 2. and qso_trim.zqso lt 2.0 and qso_trim.zqso gt 1.5, n1)
...
;;
   plate1=2942 & fiber1=393 & zabs1=0.876568
;; plate1=1746 & fiber1=382 & zabs1=1.79996
;; plate1=2630 & fiber1=73 & zabs1=1.20407
;; plate1=1510 & fiber1=440 & zabs1=1.32910
;; plate1=2328 & fiber1=43 & zabs1=1.18826
...
for j=0L, n1-1L do begin
    ...
    djs_oplot, x, ynmf, color='blue', thick=thick, linestyle=2
...
psfile = qapath+'/'+'JHU_examples_bad_'+string(nmfver, format='(I3.3)')+'.ps'
...
;; page 1: 1 absorber
    ...
    zabs = zabs1
    spec = jhusdss_decompose_loadspec(plate, fiber, nmfver)
    objname = hogg_iau_name(spec.ra, spec.dec, 'SDSS')+' (z='+string(spec.z,format='(f5.3)')+')'

    x = spec.wave*(1.+spec.z)
    ii = where(spec.ivar gt 0. and x gt 3820)
    jj = where(x lt 9250.)
    yflux = smooth(spec.flux, 3)
    yresi = smooth(spec.residual, 3)
;   ynmf = smooth(spec.nmf_continuum, 30)
;   ymed = smooth(spec.med_continuum, 30)
;   yflux = spec.flux
;   yresi = spec.residual
    ...
    pos = [0.10, 0.60, 0.90, 0.90]
    djs_plot, x[ii], yflux[ii], xra=xra, xstyle=1, $
        yra=yra, ystyle=1, position=pos, xtickformat='(A1)', ytitle=ytitle, $
        charsize=charsize, charthick=charthick, $
        xthick=thick, ythick=thick, thick=1, title=objname
    djs_oplot, x[ii], ynmf[ii], color='blue', thick=thick, linestyle=2
    djs_oplot, x[ii], ynmf[ii]*ymed[ii], color='red', thick=3
    print, 'linestyle'


    ;; overplot windows
    xpoly = [1550.*(1.+spec.z+0.02), 1550.*(1.+spec.z+0.02), 2803.*(1.+spec.z-0.04), 2803.*(1.+spec.z-0.04)]
;   xpoly = [1550.*(1.+spec.z+0.02), 1550.*(1.+spec.z+0.02), 9200., 9200.]
...
;       !y.crange[0]+[0.8, 1]*(!y.crange[1]-!y.crange[0]), $
    ...
    djs_xyouts, 1550.*(1.+spec.z-0.10), !y.crange[0]+0.55*(!y.crange[1]-!y.crange[0]), $
        'C_{ }IV', charsize=charsize, charthick=charthick, color='dark green'
    djs_xyouts, 1909.*(1.+spec.z-0.06), !y.crange[0]+0.38*(!y.crange[1]-!y.crange[0]), $
        'C_{ }III]', charsize=charsize, charthick=charthick, color='dark green'
    djs_xyouts, 2800.*(1.+spec.z-0.04), !y.crange[0]+0.40*(!y.crange[1]-!y.crange[0]), $
        'Mg_{ }II', charsize=charsize, charthick=charthick, color='dark green'
...
;       'Search Window', charsize=charsize, charthick=charthick, color='dark green'
    djs_xyouts, 2000.*(1.+spec.z), !y.crange[0]+0.88*(!y.crange[1]-!y.crange[0]), $
        'Search Window', charsize=charsize, charthick=charthick, color='dark green'

;   djs_xyouts, 8100., !y.crange[0]+0.32*(!y.crange[1]-!y.crange[0]), $
;       objname, charsize=charsize, charthick=charthick, color='black'

    items = [textoidl('NMF \times Median Continuum'), 'NMF Continuum']
    ...
    legend, items, textcolor=colors, colors=colors, $
        ...
        linestyle=[0,2], thick=thick, pspacing=1.6, pos=[9300., 4.2]
...
;       linestyle=[0,0], thick=thick, pspacing=1.6
    ...
    djs_oplot, x[ii], ymed[ii], color='orange', thick=thick+2
;   items = ['NMF Continuum', 'Median Continuum']
;   colors = [djs_icolor('red'), djs_icolor('blue')]
;   legend, items, textcolor=colors, colors=colors, $
;       box=0, /right, /top, charsize=charsize, charthick=charthick, $
;       linestyle=[0,0], thick=thick, pspacing=1.6
    ...
    items = ['Median Continuum']
    colors = [djs_icolor('orange')]
    legend, items, textcolor=colors, colors=colors, $
        box=0, /right, /bottom, charsize=charsize, charthick=charthick, $
        linestyle=[0], thick=thick, pspacing=1.6
    djs_oplot, !x.crange, [1,1], color='gray', thick=thick, linestyle=2

    djs_oplot, !x.crange, [1,1], color='gray', thick=thick, linestyle=2

    ytitle = textoidl('Final Residual')
    pos = [0.10, 0.10, 0.90, 0.35]
    yra = [0.0, 2.3]
...
;       jhusdss_qaplot_oplotlines, zabstmp, thiscolor='dark green'
        ...
        for iline=0L, n_elements(linewave)-1L do begin
            djs_xyouts, linewave[iline]*(1.+zabstmp-0.03), $
                ...
                linename[iline], color='dark green', charsize=0.7, charthick=1.6
        endfor
        djs_xyouts, 3000.*(1.+zabstmp), $
                !y.crange[0]+0.85*(!y.crange[1]-!y.crange[0]), $
                '(z_{abs}='+string(zabstmp, format='(f5.3)')+')', color='dark green', charsize=1.1, charthick=1.6
    endfor
