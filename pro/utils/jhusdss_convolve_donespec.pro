...
function jhusdss_convolve_donespec, nmfver, boss=boss

if (n_elements(nmfver) eq 0) then message, "NMF version required."
if (keyword_set(boss)) then begin
    path = jhusdss_get_path(/nmfqso)+'/'+string(nmfver, format='(I3.3)')+'/Convolve_BOSS/*/'
endif else begin
    path = jhusdss_get_path(/nmfqso)+'/'+string(nmfver, format='(I3.3)')+'/Convolve/*/'
endelse
