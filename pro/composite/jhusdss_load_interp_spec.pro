...
;-
pro jhusdss_load_interp_spec, objs, minwave=minwave, maxwave=maxwave, $
    ...
    allflux=allflux, allivar=allivar, allmask=allmask, boss=boss, dr12=dr12, $
    zuse=zuse

if (n_elements(objs) eq 0) then begin
   doc_library, 'jhusdss_load_interp_spec'
   return
endif
...
if (n_elements(path) eq 0) then begin
    if (keyword_set(dr12)) then begin
        path = jhusdss_get_path(/dr12_spectra)
    endif else begin
...
for i=0L, nobj-1L do begin
    ...
    if (keyword_set(boss) or keyword_set(dr12)) then finalpath = path+'/'+string(objs[i].(pindex), format='(i4.4)')
;   stop
