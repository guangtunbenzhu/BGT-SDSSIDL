;+
;-
pro jhusdss_detect_absorbers_montecarlo_qso2abs, nmfver, overwrite=overwrite, boss=boss 
...
outfile = path+'/OnlyAbsorbers_Window_'+jhusdss_absorbers_filename(nmfver, /mgii, boss=boss)

if (keyword_set(boss)) then begin
    qsopath=jhusdss_get_path(/nmfqso)+'/MC_'+string(nmfver, format='(I3.3)')+'/Decompose_BOSS'
endif else begin
    qsopath=jhusdss_get_path(/nmfqso)+'/MC_'+string(nmfver, format='(I3.3)')+'/Decompose'
endelse

if (jhusdss_direxist(qsopath) eq 0) then message, "Can't find the directory."
statfile = qsopath+'/'+jhusdss_stat_filename(nmfver, boss=boss)
