;+
;-
pro jhusdss_detect_absorbers_montecarlo_all, nmfver, overwrite=overwrite, iprocess=iprocess, $
    nprocess=nprocess, noparal=noparal, nodofinal=nodofinal, boss=boss
...
if (~keyword_set(noparal)) then $
   outfile = path+'/Pro_'+string(iprocess, format='(i1.1)')+'_'+jhusdss_absorbers_filename(nmfver, /mgii, boss=boss) $
else $
   outfile = path+'/'+jhusdss_absorbers_filename(nmfver, /mgii, boss=boss)
...
for i=ibegin, iend do begin
   ...
   spec = jhusdss_convolve_montecarlo_loadspec(mgii[i].plate, mgii[i].fiber, nmfver, boss=boss, error=error)
   if error then begin
      splog, "Can't find the convolved spectrum."
      absorbers[j].isitconvolved = 0b
      continue
   endif
   orispec = jhusdss_decompose_montecarlo_loadspec(mgii[i].plate, mgii[i].fiber, nmfver, boss=boss, error=error)
...
;  absorbers[j].zqso  = mgii[i].z
   absorbers[j].zqso  = mgii[i].zqso
;  if (~keyword_set(boss)) then absorbers[j].err_zqso  = mgii[i].zerr
