...
; 01/19/2012, move to master catalog
;-
pro jhusdss_detect_absorbers_train, nmfver, overwrite=overwrite, iprocess=iprocess, $
    nprocess=nprocess, noparal=noparal, dofinal=dofinal
...
if (~keyword_set(noparal)) then $
   outfile= path+'/Pitts_Pro_'+string(iprocess, format='(i1.1)')+'_'+jhusdss_absorbers_filename(nmfver, /mgii, boss=boss) $
else $
...
for i=ibegin, iend do begin
   ...
   absorbers0 = jhusdss_detect_absorbers_newengine(spec, orispec, lines=lines, nabsmax=nabsmax, dofinal=dofinal)
   ...
   absorbers[j].zqso  = spec.z
   absorbers[j].err_zqso  = mgii[i].err_zqso
