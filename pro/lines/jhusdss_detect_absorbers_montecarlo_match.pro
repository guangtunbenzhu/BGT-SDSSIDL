...
pro jhusdss_detect_absorbers_montecarlo_match, nmfver, overwrite=overwrite
...
pittspath= jhusdss_get_path(/nmfqso)+'/MC_'+string(nmfver, format='(I3.3)')
pittsfile = pittspath+'/'+'MC_Master_absorbers.fits'
pitts_abs = mrdfits(pittsfile, 2)

stat_pittspath= jhusdss_get_path(/nmfqso)+'/MC_'+string(nmfver, format='(I3.3)')+'/Decompose'
statfile = stat_pittspath+'/'+jhusdss_stat_filename(nmfver, boss=boss)
pitts_qso = mrdfits(statfile, 1)

pitts = struct_addtags(pitts_qso, pitts_abs)

npitts = n_elements(pitts)

;; the corresponding JHU catalog

jhupath= jhusdss_get_path(/nmfqso)+'/MC_'+string(nmfver, format='(I3.3)')+'/Absorbers'
infile = jhupath+'/OnlyAbsorbers_Window_'+jhusdss_absorbers_filename(nmfver, /mgii)
