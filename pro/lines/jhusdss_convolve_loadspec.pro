...
function jhusdss_convolve_loadspec, plate, fiber, nmfver, boss=boss, dr12=dr12, error=error

;; path
if (keyword_set(dr12)) then begin
   path=jhusdss_get_path(/nmfqso)+'/'+string(nmfver, format='(I3.3)')+'/Convolve_DR12/'+string(plate, format='(i4.4)')
endif else begin
   if (keyword_set(boss)) then begin
       path=jhusdss_get_path(/nmfqso)+'/'+string(nmfver, format='(I3.3)')+'/Convolve_BOSS/'+string(plate, format='(i4.4)')
;      path='/export/scratch1/menard/gz323/SDSS/QSO/NMF/'+string(nmfver, format='(I3.3)')+'/Convolve_BOSS/'+string(plate, format='(i4.4)')
   endif else begin
      path=jhusdss_get_path(/nmfqso)+'/'+string(nmfver, format='(I3.3)')+'/Convolve/'+string(plate, format='(i4.4)')
