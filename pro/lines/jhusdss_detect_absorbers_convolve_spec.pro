pro jhusdss_detect_absorbers_convolve_spec, objs0, nmfver, $
...
if (n_elements(waveminmax) eq 0) then waveminmax = jhusdss_sdsswave_minmax(boss=boss, dr12=dr12)

;; path
if (~keyword_set(path)) then begin
   if (keyword_set(dr12)) then begin
       path=jhusdss_get_path(/nmfqso)+'/'+$
          string(nmfver, format='(I3.3)')+'/Convolve_DR12'
   endif else begin
...
for ispec=0L, nspec-1L do begin
    ...
    spec = jhusdss_decompose_loadspec(obj.plate, obj.fiber, nmfver, boss=boss, dr12=dr12, error=error)
