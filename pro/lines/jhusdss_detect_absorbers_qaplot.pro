...
pro jhusdss_detect_absorbers_qaplot, spec, absorbers0, lines, writeout=writeout, path=path, boss=boss
...
if (keyword_set(writeout)) then begin
   ...
   if (~keyword_set(boss)) then begin
      filename = 'Absorber_'+string(spec.plate, format='(i4.4)')+'_'+$
                          string(spec.fiber, format='(i4.4)')+'.ps'
   endif else begin
      filename = 'Absorber_BOSS_'+string(spec.plate, format='(i4.4)')+'_'+$
                          string(spec.fiber, format='(i4.4)')+'.ps'
   endelse
   print, filename
...
for iline=0L, n_elements(lines)-1L do djs_oplot, $
    replicate(lines[iline].wave*(1.+absorbers0.zabs[0]),2), $
    !y.crange[0]+[0.6, 1.0]*(!y.crange[1]-!y.crange[0]), $
    color='red', thick=thick, linestyle=0
if (absorbers0.zabs[1] gt 0.) then begin
   for iline=0L, n_elements(lines)-1L do djs_oplot, $
       replicate(lines[iline].wave*(1.+absorbers0.zabs[1]),2), $
       !y.crange[0]+[0.6, 1.0]*(!y.crange[1]-!y.crange[0]), $
       color='dark green', thick=thick, linestyle=2
endif
if (absorbers0.zabs[2] gt 0.) then begin
   for iline=0L, n_elements(lines)-1L do djs_oplot, $
       replicate(lines[iline].wave*(1.+absorbers0.zabs[2]),2), $
       !y.crange[0]+[0.6, 1.0]*(!y.crange[1]-!y.crange[0]), $
       color='blue', thick=thick, linestyle=1
endif
if (absorbers0.zabs[3] gt 0.) then begin
   for iline=0L, n_elements(lines)-1L do djs_oplot, $
       replicate(lines[iline].wave*(1.+absorbers0.zabs[3]),2), $
       !y.crange[0]+[0.6, 1.0]*(!y.crange[1]-!y.crange[0]), $
       color='cyan', thick=thick, linestyle=1
endif

items = ['Absorber_1 z='+string(absorbers0.zabs[0], format='(f5.3)'), $
         'Absorber_2 z='+string(absorbers0.zabs[1], format='(f5.3)'), $
         'Absorber_3 z='+string(absorbers0.zabs[2], format='(f5.3)'), $
         'Absorber_4 z='+string(absorbers0.zabs[3], format='(f5.3)')]
...
djs_plot, spec.wave[ii], smooth(spec.residual[ii], 5), xra=[3700., 9500.]/(1.+spec.z), $
          yra=[0.3, 1.4], xstyle=1, ystyle=1, position=[0.10, 0.10, 0.95, 0.48], /noerase, $
...
djs_xyouts, lines[1].wave*(1.+absorbers0.zabs[0])/(1.+spec.z)-30., $
    !y.crange[0]+0.05*(!y.crange[1]-!y.crange[0]), 'Mg II', $
    color='red', charsize=charsize, charthick=charthick
djs_xyouts, lines[3].wave*(1.+absorbers0.zabs[0])/(1.+spec.z)-30., $
    !y.crange[0]+0.05*(!y.crange[1]-!y.crange[0]), 'Fe II', $
    color='red', charsize=charsize, charthick=charthick
djs_xyouts, lines[5].wave*(1.+absorbers0.zabs[0])/(1.+spec.z)-30., $
    !y.crange[0]+0.05*(!y.crange[1]-!y.crange[0]), 'Fe II', $
    color='red', charsize=charsize, charthick=charthick
