...
;minrad = minrad_tmp
;maxrad = maxrad_tmp
;minrad_tmp = 10.^(alog10(0.015)+findgen(13)*(0.5*alog10(2.0)))
;maxrad_tmp = minrad_tmp*2.0
;minrad = [[0.003], minrad_tmp]
;maxrad = [[0.015], maxrad_tmp]

minrad_tmp = 10.^(alog10(0.030)+findgen(29)*(0.5*alog10(1.5)))
maxrad_tmp = minrad_tmp*1.5
minrad = [[0.010], minrad_tmp]
maxrad = [[0.030], maxrad_tmp]

;minrad_tmp = 10.^(alog10(0.020)+findgen(17)*(0.5*alog10(1.5)))
;maxrad_tmp = minrad_tmp*1.5
;minrad = [[0.010], minrad_tmp]
...
if (massive) then outfile = repstr(outfile, '.fits', '_highmass.fits') else outfile = repstr(outfile, '.fits', '_lowmass.fits')
...
;   sfr_tmp = sfr[match.index_lrg].avg
    ...
    if (~massive) then iall = where((zgal gt zgalmin) and (zgal lt zgalmax) $
             and ((zgal gt (wave_exclude+15.)/(line_wave[0]-0.)-1.) or (zgal lt (wave_exclude-15.)/(line_wave[1]+0.)-1.)) $
             and (snr_tmp gt snrcut) $
             and (mass_tmp gt 10. and mass_tmp lt 11.3) $; and sfr_tmp gt -3. and sfr_tmp lt 3. and $
;                 ((mass_tmp lt 10.1 and sfr_tmp gt -1.29+0.65*(mass_tmp-10.)+0.5) or $
;                 (mass_tmp lt 10.7 and sfr_tmp lt -1.29+0.65*(mass_tmp-10.)+0.5))) $
             and zuniq $
;            and ((1600.*(zqso+1.) lt 3930.*(1.+zgal) and 2800.*(zqso+1.) gt 3980.*(1.+zgal) and sdev_red_tmp lt 0.1) $
    ...
    if (massive) then iall = where((zgal gt zgalmin) and (zgal lt zgalmax) $
             and ((zgal gt (wave_exclude+15.)/(line_wave[0]-0.)-1.) or (zgal lt (wave_exclude-15.)/(line_wave[1]+0.)-1.)) $
             and (snr_tmp gt snrcut) $
             and (mass_tmp gt 11.4 and mass_tmp lt 13.0) $; and sfr_tmp gt -3. and sfr_tmp lt 3. and $
;                 ((mass_tmp gt 10.1 and sfr_tmp gt -1.29+0.65*(mass_tmp-10.)+0.5) or $
;                 (mass_tmp gt 10.7 and sfr_tmp lt -1.29+0.65*(mass_tmp-10.)+0.5))) $
             and zuniq $
;            and ((1600.*(zqso+1.) lt 3930.*(1.+zgal) and 2800.*(zqso+1.) gt 3980.*(1.+zgal) and sdev_red_tmp lt 0.1) $
             and ((2830.*(zqso+1.) lt (line_wave[0]-5.)*(1.+zgal) and 3920.*(zqso+1.) gt (line_wave[1]+5.)*(1.+zgal) and sdev_red_tmp lt 0.1) $
              or  (1930.*(zqso+1.) lt (line_wave[0]-5.)*(1.+zgal) and 2770.*(zqso+1.) gt (line_wave[1]+5.)*(1.+zgal) and sdev_red_tmp lt 0.1) $
    ...
    print, 'npairs = ', nmatch
    print, '<mass> = ', median(mass_tmp[iall])
    print, '<rp> = ', mean(rp_tmp[iall])
...
;   y = fmedian
    ...
    terror = sqrt((moment(y[iuse], /nan))[1])
