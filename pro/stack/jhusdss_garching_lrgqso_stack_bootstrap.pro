...
outfile = repstr(outfile, '.fits', '_bootstrap.fits')
...
outstr = replicate({wave:outwave, fluxmean:fltarr(nwave, nbootstrap), fluxmedian:fltarr(nwave, nbootstrap), fluxgeomean:fltarr(nwave, nbootstrap), $
...
;   print, 'Make Composite ...'
    inresidual_tmp = inresidual[newindex, *]
    inivar_tmp = inivar[newindex,*]
    jhusdss_composite_engine, inresidual_tmp, inivar_tmp, fmean=fmean, fmedian=fmedian, $
       fgeomean=fgeomean, nobjuse=nobjuse, ivarweight=ivarweight, weight=weight, sigma_cut=sigma_cut

    tmp = moment(fmedian, sdev=sdev)

    outstr[irad].fluxmean[*,iboot] = fmean
    outstr[irad].fluxmedian[*,iboot] = fmedian
    outstr[irad].fluxgeomean[*,iboot] = fgeomean
endfor
