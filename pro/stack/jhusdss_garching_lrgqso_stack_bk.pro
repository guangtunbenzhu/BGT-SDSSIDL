...
;minrad = [0.020, 0.040, 0.050, 0.100, 0.150, 0.300, 0.600];, 0.180];, 0.200];, 0.170, 0.180];, 0.150]
;maxrad = [0.040, 0.080, 0.100, 0.200, 0.300, 0.600, 1.200];, 0.360];, 0.400];, 0.340, 0.360];, 0.300]

minrad_tmp = 10.^(alog10(0.030)+findgen(29)*(0.5*alog10(1.5)))
;minrad_tmp = 10.^(alog10(0.030)+findgen(12)*(0.5*alog10(1.5)))
...
;   y = fmedian
    y = fgeomean
    inoline = where(outwave lt line_wave[0]-5. or outwave gt line_wave[1]+5., comp=iline)
    med_continuum = y*0.+1.
    med_continuum[inoline] = median(y[inoline], 21, /even)
;   med_continuum[iline] = median(y[inoline], 21, /even)
;   y = (y/med_continuum)
...
;   stop
    ...
    jhusdss_singlet_smooth, 1.-clevel-y, fltarr(n_elements(y))+1./terror^2, $
       outflux=single_newy, outivar=single_newyivar, sigma=2.0
...
;   single_newyivar = fltarr(n_elements(y))+1./terror^2
    jhusdss_doublet_smooth, 1.-clevel-y, fltarr(n_elements(y))+1./terror^2, $
       outflux=newy, outivar=newyivar, sigma=2.0, separation=fix_separation
    ...
    djs_oplot, 2853.*[1,1.], !y.crange[0]+[0.5,0.9]*(!y.crange[1]-!y.crange[0])
...
;   djs_plot, outstr.rp, outstr.ew_caii_3934, psym=4
