...
;for irad=0L, 3L do begin
    ...
    iall = where((zgal gt zgalmin) and (zgal lt zgalmax) $
             ...
             and ((2400.*(zlrg+1.) lt (line_wave[0]-5.)*(1.+zgal) and (2700.*(zlrg+1.) gt (line_wave[1]-5.)*(1.+zgal) and sdev_blue_tmp lt sdevcut)) $
              ...
              or  (4050.*(zlrg+1.) lt (line_wave[0]-5.)*(1.+zgal) and (4250.*(zlrg+1.) gt (line_wave[1]+5.)*(1.+zgal) and sdev_blue_tmp lt sdevcut)) $
              or  (4350.*(zlrg+1.) lt (line_wave[0]-5.)*(1.+zgal) and (5050.*(zlrg+1.) gt (line_wave[1]+5.)*(1.+zgal) and sdev_blue_tmp lt sdevcut)) $
              or  (5350.*(zlrg+1.) lt (line_wave[0]-5.)*(1.+zgal) and (5850.*(zlrg+1.) gt (line_wave[1]+5.)*(1.+zgal) and sdev_blue_tmp lt sdevcut))), $
             nall)

    match = match[iall]
    nmatch = n_elements(match)

    spec_snr = stat[match.index_lrg].spec_snr_median
...
;   Need to reformat residuals
;   print, 'Get residuals ...'
;   for i=0L, nwave-1L do inresidual[*,i] = allspec.residual[match.index_lrg, icaii-wave_shift+i]
;   for i=0L, nwave-1L do inresidual[*,i] = allspec.subtracted_residual[match.index_lrg, icaii-wave_shift+i]
;   print, 'Get ivar...'
;   for i=0L, nwave-1L do inivar[*,i] = allspec.residual_ivar[match.index_lrg, icaii-wave_shift+i]
;   for i=0L, nwave-1L do inivar[*,i] = allspec.subtracted_ivar[match.index_lrg, icaii-wave_shift+i]
