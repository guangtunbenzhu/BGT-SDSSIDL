...
i_indep = i_indep[0:11]

;i_indep = [lindgen(10)*2+2]
...
nran = 12
iran = ((long(randomu(seed, 50)*200) > 0) < 249)
isort = bsort(iran)
iran = iran[isort[uniq(isort)]]
iran = iran[lindgen(nran)*3+1]

iran = [2,27,32,50,61,88,92,99,106,111,123,138]
...
; !p.multi=[0,3,4]
  ...
  for i=0,11 do begin
      ...
      djs_oplot, a[i_indep].rp, a[i_indep].ew_nofit_2_mc[iran[i]]/a[i_indep].sdev_nofit_2_mc, psym=symcat(14), symsize=1.8, thick=thick, color=zcolor[i]
      djs_oplot, a[i_indep].rp, a[i_indep].ew_nofit_2_mc[iran[i]]/a[i_indep].sdev_nofit_2_mc, thick=10, color=zcolor[i]
      djs_oplot, xra, [0, 0], color='grey'
      ...
      if (i eq 2) then legend, [textoidl('Measurement at Ca II \lambda_K'), textoidl('Null tests at random \lambda')], $
         psym=[8,symcat(14)], colors=[djs_icolor('blue'), djs_icolor('dark gray')], thick=8, charsize=1.5, charthick=charthick, $
         /top, /right, box=0, textcolors=[djs_icolor('blue'), djs_icolor('dark gray')], pos=[1100,5.7]
      plotsym, 0, /fill
...
;         charsize=charsize-0.5, charthick=charthick, color='dark gray'
  ...
  djs_oplot, a[i_indep].rp, a[i_indep].ew_nofit_2/a[i_indep].sdev_nofit_2_mc, psym=8, symsize=3, thick=thick, color='blue'
