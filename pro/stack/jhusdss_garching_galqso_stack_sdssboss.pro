...
minrad = [0.050, 0.060, 0.070, 0.080, 0.090, 0.100, 0.120, 0.135, 0.150]
maxrad = [0.100, 0.120, 0.140, 0.160, 0.180, 0.200, 0.240, 0.270, 0.300]
...
;            and ((1600.*(zqso+1.) lt 3930.*(1.+zgal) and 2800.*(zqso+1.) gt 3980.*(1.+zgal) and sdev_red_tmp lt 0.1) $
    ...
    icaii = value_locate(allspec.wave, line_wave[0]*(1.+newzgal))
...
;   mass_tmp = mass[match.index_gal].avg
    ...
    boss_iall = where((boss_zgal gt zgalmin) and (boss_zgal lt zgalmax) $
             ...
             and boss_zuniq $
;            and ((1600.*(zqso+1.) lt 3930.*(1.+zgal) and 2800.*(zqso+1.) gt 3980.*(1.+zgal) and sdev_red_tmp lt 0.1) $
             and ((1600.*(boss_zqso+1.) lt (line_wave[0]-5.)*(1.+boss_zgal) and boss_sdev_red_tmp lt sdevcut) $
              or  (1250.*(boss_zqso+1.) lt (line_wave[0]-5.)*(1.+boss_zgal) and 1500.*(boss_zqso+1.) gt (line_wave[1]+5.)*(1.+boss_zgal) and boss_sdev_blue_tmp lt sdevcut)), $
             boss_nall)
    ...
    boss_icaii = value_locate(boss_allspec.wave, line_wave[0]*(1.+boss_newzgal))
    ...
    jhusdss_doublet_smooth, 1.-clevel-y, fltarr(n_elements(y))+1./terror^2, $
       outflux=newy, outivar=newyivar, sigma=1.5
