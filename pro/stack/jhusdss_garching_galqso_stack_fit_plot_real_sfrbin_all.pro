...
if (DoScale) then rvir=228. else rvir=100.
...
quiescent = 1b 
infile = infile0
if (quiescent) then infile = repstr(infile, '.fits', '_quiescent_sfr.fits') else infile = repstr(infile, '.fits', '_sf_sfr.fits')
...
psfile1 = repstr(infile1, '.fits', '_fit_realsfrbin_1.ps')
psfile2 = repstr(infile1, '.fits', '_fit_realsfrbin_2.ps')
psfile3 = repstr(infile1, '.fits', '_fit_realsfrbin_3.ps')
psfile4 = repstr(infile1, '.fits', '_fit_realsfrbin_4.ps')
psfile5 = repstr(infile1, '.fits', '_fit_realsfrbin_5.ps')
psfile6 = repstr(infile1, '.fits', '_fit_realsfrbin_6.ps')
...
jhusdss_powerlaw_fit, a_sf[ifit].rp/rvir, a_sf[ifit].ew_nofit_2, sdev_nofit_2_mc[ifit], in_slope=fid_slope, in_intercept=10.^fid_intercept, $
...
jhusdss_powerlaw_fit, a_quiescent[ifit].rp/rvir, a_quiescent[ifit].ew_nofit_2, sdev_nofit_2_mc[ifit], in_slope=fid_slope, in_intercept=10.^fid_intercept, $
...
; djs_axis, xaxis=0, xthick=xthick, xtickformat='(A1)'
  ...
  djs_oplot, xx, 10.^(alog10(xx/rvir)*sf_slope+sf_intercept), thick=thick+2, color='navy', linestyle=0
  djs_oplot, xx, 10.^(alog10(xx/rvir)*quiescent_slope+quiescent_intercept), thick=thick+2, color='red', linestyle=2
...
;      charsiz=charsize, charthick=charthick
  ...
  plotsym, 0, /fill
  djs_xyouts, 45, 4E-1, 'Star-forming (HSF+LSF)', color='navy', charsize=charsize-0.2, charthick=charthick
  djs_oplot, 250*[1,1], 4.5E-1*[1,1], color='navy', psym=8, symsize=1.3, thick=thick
  plotsym, 4, thick=thick
  djs_xyouts, 45, 4E-1*0.77^2, '      Quiescent (HQ+LQ)', color='red', charsize=charsize-0.2, charthick=charthick
