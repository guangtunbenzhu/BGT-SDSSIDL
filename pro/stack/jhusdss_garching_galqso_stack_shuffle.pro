...
nqso = n_elements(qso)
ihigh_snr = where(stat.spec_snr_median gt snrcut $
             and ((2830.*(qso.z+1.) lt (line_wave[0]-5.)*(1.+0.1) and 3920.*(qso.z+1.) gt (line_wave[1]+5.)*(1.+0.1) and stat.med_sdeviation_red lt 0.1) $
              or  (1250.*(qso.z+1.) lt (line_wave[0]-5.)*(1.+0.1) and 1520.*(qso.z+1.) gt (line_wave[1]+5.)*(1.+0.1) and stat.med_sdeviation_blue lt 0.1)) $
              , nhigh_snr)

for ishuffle = 4, 5 do begin
...
;            and ((1600.*(zqso+1.) lt 3930.*(1.+zgal) and 2800.*(zqso+1.) gt 3980.*(1.+zgal) and sdev_red_tmp lt 0.1) $
    ...
    ;; shuffle the index
;   newindex = long(randomu(seed, nmatch)*(nmatch-1))
;   newindex = ((newindex>0)<(nmatch-1))
;   newindex= match[newindex].index_qso

    newindex = long(randomu(seed, nmatch)*(nhigh_snr-1))
    newindex = ((newindex>0)<(nhigh_snr-1))
    newindex = ihigh_snr[newindex]

    print, 'Get residuals ...'
    for i=0L, nwave-1L do inresidual[*,i] = allspec.residual[newindex, icaii-wave_shift+i]
    print, 'Get ivar...'
    for i=0L, nwave-1L do inivar[*,i] = allspec.residual_ivar[newindex, icaii-wave_shift+i]
