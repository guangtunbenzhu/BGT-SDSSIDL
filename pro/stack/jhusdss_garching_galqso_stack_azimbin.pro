...
;minrad_tmp = 10.^(alog10(0.010)+findgen(17)*(0.5*alog10(1.5)))
;maxrad_tmp = minrad_tmp*1.5
;minrad = minrad_tmp
;maxrad = maxrad_tmp

;minrad_tmp = 10.^(alog10(0.015)+findgen(8)*(0.5*alog10(2.0)))
;maxrad_tmp = minrad_tmp*2.0
;minrad = [[0.005], minrad_tmp]
;maxrad = [[0.015], maxrad_tmp]

;minrad_tmp = 10.^(alog10(0.020)+findgen(15)*(0.5*alog10(1.5)))
;maxrad_tmp = minrad_tmp*1.5
...
if (Polar) then outfile = repstr(outfile, '.fits', '_Polar.fits') else outfile = repstr(outfile, '.fits', '_Disk.fits')
...
if choice_load_data eq 1 then begin
   ...
   match0_azimuthal = jhusdss_galqso_match_readin(boss=boss, /azimuthal)
endif
...
for irad=0L, nrp-1L do begin
    ...
    azimuthal_tmp = match0_azimuthal[isub].azimuthal
    phi_tmp = match0_azimuthal[isub].phi
    ab_tmp = match0_azimuthal[isub].ab
    ...
    if (~Polar) then iall = where((zgal gt zgalmin) and (zgal lt zgalmax) $
             ...
             and (ab_tmp gt 0.01 and ab_tmp lt ab_cut and phi_tmp gt -90. and phi_tmp lt 90. and $
                  (abs(azimuthal_tmp) gt 45. and abs(azimuthal_tmp) lt 135.)) $
             and zuniq $
;            and ((1600.*(zqso+1.) lt 3930.*(1.+zgal) and 2800.*(zqso+1.) gt 3980.*(1.+zgal) and sdev_red_tmp lt 0.1) $
    ...
    if (Polar) then iall = where((zgal gt zgalmin) and (zgal lt zgalmax) $
             ...
             and (mass_tmp gt 7. and mass_tmp lt 13. and sfr_tmp gt -3. and sfr_tmp lt 3.) $
             and (ab_tmp gt 0.01 and ab_tmp lt ab_cut and phi_tmp gt -90. and phi_tmp lt 90. and $
                  (abs(azimuthal_tmp) lt 45. or abs(azimuthal_tmp) gt 135.)) $
             and zuniq $
;            and ((1600.*(zqso+1.) lt 3930.*(1.+zgal) and 2800.*(zqso+1.) gt 3980.*(1.+zgal) and sdev_red_tmp lt 0.1) $
             and ((2830.*(zqso+1.) lt (line_wave[0]-5.)*(1.+zgal) and 3920.*(zqso+1.) gt (line_wave[1]+5.)*(1.+zgal) and sdev_red_tmp lt 0.1) $
              or  (1930.*(zqso+1.) lt (line_wave[0]-5.)*(1.+zgal) and 2770.*(zqso+1.) gt (line_wave[1]+5.)*(1.+zgal) and sdev_red_tmp lt 0.1) $
