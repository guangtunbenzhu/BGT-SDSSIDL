...
k_print, filename=psfile1, axis_char_scale=1.5, xsize=8, ysize=8
  ...
  !y.margin=0
  for i=0,8 do begin
      shuffle_infile = repstr(infile, '_fit.fits', '_shuffle_'+string(i+4, format='(i2.2)')+'_fit.fits')
      ashuffle = mrdfits(shuffle_infile, 1)
...
; !p.multi=[0,3,4]
  ...
  for i=0,11 do begin
      ...
      shuffle_infile = repstr(infile, '_fit.fits', '_shuffle_'+string(ichoose[i], format='(i2.2)')+'_fit.fits')
      ashuffle = mrdfits(shuffle_infile, 1)
      ...
      if (i eq 2) then legend, [textoidl('Measurement at Ca II \lambda_K'), textoidl('Null tests using random quasars')], $
