...
gi_outfile = repstr(outfile, '.fits', '_deltagi.fits')
...
if (DoNaI) then begin
   gi_outfile=repstr(gi_outfile,'.fits','_NaI.fits')
endif
if (DoScale) then begin
   gi_outfile=repstr(gi_outfile,'.fits','_Scale.fits')
endif
if (DoLargeScale) then begin
   gi_outfile=repstr(gi_outfile,'.fits','_LargeScale.fits')
endif
...
outstr = replicate({delta_gi_mean:0., delta_gi_median:0., $
...
for irad=0L, nrp-1L do begin
    ...
    imag_tmp = qso_gi[match.index_qso].ugriz[3]
...
;            and ((1600.*(zqso+1.) lt 3930.*(1.+zgal) and 2800.*(zqso+1.) gt 3980.*(1.+zgal) and sdev_red_tmp lt 0.1) $
    ...
    iuse_delta = where(newdelta_gi gt -5. and newdelta_gi lt 5.)
    outstr[irad].delta_gi_mean = mean(newdelta_gi[iuse_delta])
    outstr[irad].delta_gi_median = median(newdelta_gi[iuse_delta])
...
;   weight = newzgal^2*100.
    if (DoWeight) then weight=1./match.rp_mpc
...
;if savemean then mwrfits, outstr, gi_outfile, /create

;for i=0,2 do BEEP
djs_plot, outstr.rp, outstr.delta_gi_median+0.0054, /xlog, /ylog, yra=[1E-6, 5E-1], yst=1, psym=4, thick=3
;djs_plot, outstr.rp, abs(outstr.delta_gi_mean-0.02895), /xlog, /ylog, yra=[1E-6, 5E-1], yst=1, psym=4, thick=3
djs_plot, outstr.rp, outstr.delta_gi_mean-0.02335, /xlog, /ylog, yra=[1E-6, 5E-1], yst=1, psym=4, thick=3

k_print, filename='delta_gi_lrg.ps', axis_char_scale=1.3, xsize=8, ysize=6
  djs_plot, outstr.rp, abs(outstr.delta_gi_mean-0.02696), /xlog, /ylog, yra=[1E-6, 1E-1], yst=1, psym=4, symsize=1.5, thick=6, $
      xthick=6, ythick=6, xtitle='r_p (kpc)', ytitle='<\Delta g-i>', charsize=1.5, charthick=3, color='red', xra=[10, 30000], xst=1, $
      ytickformat='jhusdss_tick_exponent', title='Reddening around LRGs'
k_end_print
