...
for ishuffle = 1L, 24L do begin
stackpath = jhusdss_get_path(/nmfqso)+'/'+string(nmfver, format='(I3.3)')+'/Stack/'
infile0 = stackpath + jhusdss_garching_lrgqso_stack_filename(nmfver, boss=boss)
infile = repstr(infile0, '.fits', '_shuffle_'+string(ishuffle, format='(i2.2)')+'.fits')
shuffle_infile = repstr(infile0, '.fits', '_shuffle_mean.fits')
...
outstr = replicate({rp:0., ew:0., err_ew:0., sigma:0., err_sigma:0., line_ratio:0., err_line_ratio:0., $
...
for irp=0L, nrp-1L do begin
    ...
    y = comp[irp].fluxgeomean/comp_shuffle[irp].fluxgeomean
...
;   y = (y/med_continuum)
    ...
    outstr[irp].err_sigma = err_sigma
...
;   mcmom_ew_nofit_lr_1_s5 = moment(outstr[irp].ew_nofit_lr_1_s5_mc[iusemc], sdev=nofit_lr_1_s5_mcsdev)
    ...
    ;; QAplots
    load_dp, /b
    print, 'vdisp: ', outstr[irp].sigma, outstr[irp].err_sigma, outstr[irp].sigma2, outstr[irp].err_sigma2
