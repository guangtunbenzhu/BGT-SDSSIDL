pro jhusdss_lowz_lrg_caii, nmfver
...
infile = lowzpath+'/'+jhusdss_lowz_lrg_composite_filename(nmfver)
outfile = lowzpath+'/EW_'+jhusdss_lowz_lrg_composite_filename(nmfver)
...
;  y = lowz[i].fluxmedian[j,*]
   ...
   jhusdss_doublet_smooth, 1.-clevel-reform(y), reform(yivar), outflux=newy, outivar=newyivar, sigma=3
   newy = 1.-newy-clevel
...
;djs_plot, lowz.rp, outstr.lflux, psym=4, /xlog, /ylog, yra=[0.0005, 1.0], xra=[5,1000], xst=1, yst=1
;oploterror, lowz.rp, outstr.lflux, outstr.err_lflux, psym=4, $
;     color=djs_icolor('red'), errcolor=djs_icolor('red')
;print, outstr.lflux/outstr.err_lflux
;stop
;stop
...
stop
i = 0
djs_plot, lowz.mass[i], outstr.lflux[i,*], psym=4, /ylog, yra=[1E-2, 2E0]
