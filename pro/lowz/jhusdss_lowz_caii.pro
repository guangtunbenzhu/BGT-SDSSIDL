...
outfile = lowzpath+'/EW_'+jhusdss_lowz_composite_filename(nmfver)
...
iwave = where(wave gt 3933. and wave lt 3935)
for i=0L, 0L do begin

   for j=0L, nrp-1L do begin

;  y = lowz[i].fluxmedian[j,*]
   y = lowz[i].fluxmean[j,*]
   ...
   djs_plot, wave, smooth(y+clevel, 5), $
       xra=xra, yra=yra, thick=thick, xthick=thick, ythick=thick, $
       xtickformat='(A1)', charsize=charsize, charthick=charthick, pos=[0.1, 0.5, 0.9, 0.9]
   ...
   tmp = moment(newy+clevel, sdev=sdev)
   plothist, newy+clevel, binval, hist, bin=sdev/10., peak=1, pos=[0.1, 0.1, 0.9, 0.5], $
      /noerase
;  jj = where(binval gt 1.-sdev and binval lt 1.+sdev)
;  djs_oplot, binval[jj], hist[jj], psym=10., color='gray'
   jj = where((newy+clevel) gt 1.-2.*sdev and (newy+clevel) lt 1.+2.*sdev)
   plothist, (newy+clevel)[jj], bin=sdev/10., peak=1, $
      /overplot, /fill, color=djs_icolor('gray')
   plothist, (newy+clevel)[iwave], bin=sdev/10., peak=1, /overplot, $
      color=djs_icolor('red'), fcolor=djs_icolor('red'), /fill
;  x_poly = [1.-sdev, 1.+sdev, 1.+sdev, 1.-sdev, 1.-sdev]
;  y_poly = [!y.crange[0], !y.crange[0], !y.crange[1], !y.crange[1], !y.crange[0]]
;  polyfill, x_poly, y_poly, /line_fill
...
stop
i = 2
djs_plot, lowz.mass[i], outstr.lflux[i,*], psym=4, /ylog, yra=[1E-4, 1E0]
