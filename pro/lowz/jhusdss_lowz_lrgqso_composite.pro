...
pro jhusdss_lowz_lrgqso_composite, nmfver, lrgver, boss=boss, overwrite=overwrite
...
if (n_elements(lrg_minrad) eq 0) then lrg_minrad=[0.0];, 0.8, 1.2, 1.6]
if (n_elements(lrg_maxrad) eq 0) then lrg_maxrad=[0.1];, 1.2, 1.6, 1.8]
;if (n_elements(minrad) eq 0) then minrad=[0.0, 0.8, 1.2, 1.6]
;if (n_elements(maxrad) eq 0) then maxrad=[0.8, 1.2, 1.6, 1.8]
...
endelse
outfile = lowzpath+'/'+jhusdss_lowz_lrgqso_composite_filename(nmfver)
...
for irad=0L, n_elements(lrg_minrad)-1L do begin
    tmpspec = jhusdss_gallrg_match_spec_readin(lrgver, lrg_minrad[irad], lrg_maxrad[irad], boss=boss, match=tmpmatch)
    if (n_elements(lrg_match) eq 0) then begin
       lrg_match=tmpmatch
       lrg_spec_allflux = tmpspec.allflux
       lrg_spec_allivar = tmpspec.allivar
    endif else begin
       lrg_match=[lrg_match, tmpmatch]
       lrg_spec_allflux = [spec_allflux, tmpspec.allflux]
       lrg_spec_allivar = [spec_allivar, tmpspec.allivar]
    endelse
endfor
;rp_all = 10.^(alog10(match.rp_mpc*1E3) - 0.20*(mass[match.index_gal].avg-10.1))
lrg_rp_all = lrg_match.rp_mpc*1E3
...
for i=0L, rp_nbin-1L do begin
    ...
    lrg_igal = where(lrg_rp_all gt rp_bin[i].min and lrg_rp_all le rp_bin[i].max, lrg_ngal)
    ...
    ;; LRG
    lrg_ssfr_tmp = ssfr[lrg_match[lrg_igal].index_gal].avg
    lrg_sfr_tmp = sfr[lrg_match[lrg_igal].index_gal].avg
    lrg_mass_tmp = mass[lrg_match[lrg_igal].index_gal].avg
    lrg_zgal = gal[lrg_match[lrg_igal].index_gal].z
    lrg_zuniq = galuniq[lrg_match[lrg_igal].index_gal].choose
    zlrg = lrg[lrg_match[lrg_igal].index_lrg].z
    lrg_rp_tmp = lrg_match[lrg_igal].rp_mpc*1E3

    lrg_iall = where(lrg_zgal gt zgalmin $
             and lrg_zgal lt zlrg-0.035 $
             and zlrg gt 0.08 $
;            and zlrg lt zlrgmax $
;            and (zlrg gt 1.02 or zlrg lt 0.95) $
;            and 1700.*(zlrg+1.) lt 3934.78*(1.+zgal) $
             and lrg_zuniq, lrg_nall)

    if (nall le 1 or lrg_nall le 1) then message, 'I have less than 2 pairs'
    print, 'N(Pairs) = ', nall

    all_ssfr = [ssfr_tmp[iall], lrg_ssfr_tmp[lrg_iall]]
    all_sfr = [sfr_tmp[iall], lrg_sfr_tmp[lrg_iall]]
    all_mass = [mass_tmp[iall], lrg_mass_tmp[lrg_iall]]
    all_zgal = [zgal[iall], lrg_zgal[lrg_iall]]
    all_zqso = [zqso[iall], zlrg[lrg_iall]]
    all_rp = [rp_tmp[iall], lrg_rp_tmp[lrg_iall]]
    allflux = [spec_allflux[igal[iall], *], lrg_spec_allflux[lrg_igal[lrg_iall],*]]
    allivar = [spec_allivar[igal[iall], *], lrg_spec_allivar[lrg_igal[lrg_iall],*]]
